...
def Deploy_10(request): 
    ...
    if request.method == "POST":
        ...
        image = Image.open("PROJECT/media/" + str(result1)).convert("RGB")
        size = (224, 224)
        ...
        a = (classes[idd])
        if a == 'Cyst':
            a = 'THE KIDNEY CANCER TYPE OF CYST AFFECTED'
            engine = pyttsx3.init()
            engine.say(a)
            engine.runAndWait()
            ...
            a = 'THE KIDNEY CANCER TYPE OF A STONE AFFECTED'
            engine = pyttsx3.init()
            engine.say(a)
            engine.runAndWait()
            
        elif a == 'Tumor':
            a = 'THE KIDNEY CANCER TYPE OF A TUMOR AFFECTED'
            engine = pyttsx3.init()
            engine.say(a)
            engine.runAndWait()
